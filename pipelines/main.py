#!/usr/bin/env python3
"""
SemGraph Main Script with Edge Weight Integration

GraphMAE2 + GCN + Edge Weight를 사용한 전체 파이프라인 실행

Usage:
    python pipelines/main.py --mode train --config config.json
    python pipelines/main.py --mode evaluate --model model.pkl
    python pipelines/main.py --mode ablation --output artifacts/runs/results/
"""
...
from core.services.semgraph import SemGraphConfig
...
def print_banner():
    ...
    banner = f"""
{Fore.CYAN}{'=' * 80}
{Fore.GREEN}  SemGraph: Semantic Graph Clustering
...
def create_default_config() -> SemGraphConfig:
    """기본 설정 생성"""
    ...
    config = SemGraphConfig(
        ...
        # 출력
        save_results=True,
        output_dir="artifacts/runs/results",
...
def run_training(config: SemGraphConfig, verbose: bool = True):
    """전체 파이프라인 학습 실행"""
    from core.services.semgraph import SemGraphPipeline

    print(f"\n{Fore.CYAN}{'=' * 80}")
    print(f"{Fore.GREEN}Starting SemGraph Pipeline")
...
def run_traditional_comparison(config: SemGraphConfig):
    """전통적 그래프 클러스터링과 SemGraph 비교"""
    from core.services.semgraph import SemGraphPipeline

    print(f"\n{Fore.CYAN}{'=' * 80}")
    print(f"{Fore.GREEN}Comparison: SemGraph vs Traditional Graph Clustering")
    print(f"{Fore.CYAN}{'=' * 80}{Style.RESET_ALL}\n")

    print(f"{Fore.YELLOW}Note: Traditional comparison is a separate analysis mode.{Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Running full SemGraph pipeline...{Style.RESET_ALL}\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="SemGraph: Semantic Graph Clustering (Edge Weight Integration)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # 기본 학습 실행
  python pipelines/main.py --mode train

  # 설정 파일로 학습
  python pipelines/main.py --mode train --config configs/my_config.json

  # GAT vs GCN+EdgeWeight 비교
  python pipelines/main.py --mode compare --max-docs 5000 --epochs 500

  # 전통적 모델과 비교 (Louvain, Leiden, etc.)
  python pipelines/main.py --mode compare-traditional --max-docs 10000

  # GPU 사용
  python pipelines/main.py --mode train --device cuda
  ...
  # 실험 프로토콜은 Phase 7에서 dataset-aware runner로 재작성 예정
    ...
    parser.add_argument(
        '--output',
        type=str,
        default='artifacts/runs/results/semgraph_gcn_edge_weight',
        help='출력 디렉토리'
    )
    ...
    if args.config:
        ...
        # TODO: config_dict로부터 SemGraphConfig 생성
